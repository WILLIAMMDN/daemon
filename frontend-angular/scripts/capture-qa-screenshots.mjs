...
const outDir = path.resolve(__dirname, '../../docs/qa/screenshots');
...
server.listen(4200, async () => {
  ...
  const setupPage = async (page) => {
    ...
    await page.route('**/api/v1/**', async (route) => {
      ...
      if (url.includes('/auth/yo') || url.includes('/usuarios/yo')) {
        return route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ usuario: mockUser, data: mockUser }) });
      }
      if (url.includes('/notificaciones')) {
        return route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify([]) });
      }
      if (url.includes('/misiones')) {
        return route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify([]) });
      }
      if (url.includes('/evaluaciones')) {
        return route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify([]) });
      }
      if (url.includes('/competencia/estado')) {
        return route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify(null) });
      }
      if (url.includes('/alumno/panel')) {
        return route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify(mockPanel) });
      }
      if (url.includes('/alumno/home-context')) {
        return route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify(mockHomeContext) });
      }
      if (url.includes('/alumno/learning-context')) {
        return route.fulfill({
          ...
          body: JSON.stringify({
            student: mockUser,
            currentEnrollment: mockHomeContext.currentEnrollment,
            activeEnrollments: [mockHomeContext.currentEnrollment],
            generatedAt: new Date().toISOString(),
          }),
        });
      }
      if (url.includes('/alumno/aprender/mapa')) {
        return route.fulfill({
          ...
          contentType: 'application/json',
          body: JSON.stringify({
            path: { id: 1, title: 'Ruta Principal' },
            milestones: [],
            nextItem: null,
      ...
      if (url.includes('/alumno/aprendizaje')) {
        return route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify(mockAprendizaje) });
      }
      if (url.includes('/alumno/agenda')) {
        return route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify(mockAgenda) });
      }
      ...
      if (url.includes('/tienda/premios')) {
        return route.fulfill({
          ...
          body: JSON.stringify({
            premios: [
              { id: 1, nombre: 'Skin Dragón', imagen: null },
              { id: 2, nombre: 'Gafas Cyber', imagen: null },
            ],
          }),
        });
      }
      if (url.includes('/cuentos') || url.includes('/proyectos')) {
        return route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify([]) });
      }
      return route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify([]) });
    });
    ...
    await page.addInitScript((user) => {
      ...
      localStorage.setItem('daemon_sidebar_alumno_colapsado', 'false');
      localStorage.setItem('daemon_sidebar_alumno_colapsado_pin', 'true');
      localStorage.setItem('daemon_sidebar_colapsado', 'false');
      localStorage.setItem('daemon_sidebar_colapsado_pin', 'true');
    }, mockUser);
  };
  ...
  /** Helper: navigate to URL, wait for Angular layout shell, then capture. */
  const nav = async (url, waitForSel) => {

    await page.goto(`http://localhost:4200${url}`);
    await page.waitForSelector('app-sidebar-portal', { timeout: 15000 });
    if (waitForSel) {
  ...
  // ── 01 Golden Student Inicio ─────────────────────────────────────────
  console.log('Capturing Screenshot 1: /alumno (Golden Student Inicio)...');
  await nav('/alumno', '.student-dashboard');
   // ── 02 /alumno — Aprender submenu open ──────────────────────────────
  console.log('Capturing Screenshot 2: /alumno with Aprender submenu open...');
  // Click the Aprender parent nav button by its visible text
  const aprenderBtn = page.locator('button.nav-parent', { hasText: 'Aprender' }).first();
  if (await aprenderBtn.count() > 0) {
    await aprenderBtn.click({ force: true });
    await page.waitForTimeout(700);
  }
  await page.screenshot({ path: path.join(outDir, '02-alumno-sidebar-aprender-open-1440.png'), fullPage: false });

  // ── 03 /alumno/aprender/mis-aprendizajes ────────────────────────────
  console.log('Capturing Screenshot 3: /alumno/aprender (Mis aprendizajes)...');
  await nav('/alumno/aprender/mis-aprendizajes', 'app-mis-aprendizajes, [class*="aprendizajes"], h1, h2');
  // Open Aprender accordion so sidebar shows active section
  const aprenderBtn3 = page.locator('button.nav-parent', { hasText: 'Aprender' }).first();
  if (await aprenderBtn3.count() > 0) {
    await aprenderBtn3.click({ force: true }).catch(() => {});
    await page.waitForTimeout(500);
  }
  await page.screenshot({ path: path.join(outDir, '03-alumno-aprender-mis-aprendizajes-1440.png'), fullPage: false });

  // ── 04 /alumno/aprender/curso/1 ─────────────────────────────────────
  console.log('Capturing Screenshot 4: /alumno/aprender/curso/1 (Learning space)...');
  await nav('/alumno/aprender/curso/1', 'app-espacio-curso, [class*="espacio"], [class*="curso"], h1, h2');
  await page.screenshot({ path: path.join(outDir, '04-alumno-aprender-espacio-curso-1440.png'), fullPage: false });

  // ── 05 /alumno/crear — Crear submenu open ───────────────────────────
  console.log('Capturing Screenshot 5: /alumno with Crear submenu open...');
  await nav('/alumno/crear/proyectos', 'app-mis-proyectos, [class*="proyectos"], h1, h2');
  // Open Crear accordion so it shows in sidebar
  const crearBtn5 = page.locator('button.nav-parent', { hasText: 'Crear' }).first();
  if (await crearBtn5.count() > 0) {
    await crearBtn5.click({ force: true }).catch(() => {});
    await page.waitForTimeout(500);
  }
  await page.screenshot({ path: path.join(outDir, '05-alumno-crear-submenu-open-1440.png'), fullPage: false });

  // ── 06 /alumno/agenda/sesiones ──────────────────────────────────────
  console.log('Capturing Screenshot 6: /alumno/agenda (Sesiones)...');
  await nav('/alumno/agenda/sesiones', 'app-sesiones, [class*="sesiones"], h1, h2');
  await page.screenshot({ path: path.join(outDir, '06-alumno-agenda-sesiones-1440.png'), fullPage: false });

  // ── 07 /alumno/identidad — Identidad submenu open ───────────────────
  console.log('Capturing Screenshot 7: /alumno/identidad with submenu open...');
  await nav('/alumno/identidad/resumen', 'app-resumen-identidad, [class*="identidad"], h1, h2');
  // Open Identidad accordion
  const idBtn7 = page.locator('button.nav-parent', { hasText: 'Identidad' }).first();
  if (await idBtn7.count() > 0) {
    await idBtn7.click({ force: true }).catch(() => {});
    await page.waitForTimeout(500);
  }
...
��───────────────
  ...
  await nav('/alumno/identidad/resumen', 'app-resumen-identidad, [class*="identidad"], h1, h2');
  // Open Identidad accordion
  await page.locator('#sidebar-alumno-identidad').click({ force: true }).catch(() => {});
  await page.waitForTimeout(500);
  await page.screenshot({ path: path.join(outDir, '07-alumno-identidad-submenu-open-1440.png'), fullPage: false });
  ...
  await mobilePage.waitForSelector('.student-dashboard', { timeout: 15000 });
  ...
  await tabletPage.waitForSelector('.student-dashboard', { timeout: 15000 });
