...
describe('Autenticacion (login local -> Firebase)', () => {
  ...
  it('guarda la sesion y vincula la identidad de Firebase tras el login local', async () => {
    api.post
      .mockReturnValueOnce(of({ usuario }))
      .mockReturnValueOnce(of({ token: 'custom-token-abc' }));
    ...
    expect(api.post).toHaveBeenNthCalledWith(2, '/auth/firebase-token', {});
  ...
  it('no rompe el login local si el endpoint de Firebase falla (best-effort)', async () => {
    api.post
      .mockReturnValueOnce(of({ usuario }))
      .mockReturnValueOnce(throwError(() => new Error('Firebase no disponible')));
