...
import { CuentoComentario, CuentoDetallePayload, CuentoPayload, CuentoReaccion, CuentoRegistro } from '../models/cuento.models';
...
export class Cuento {
  ...
  detalle(id: string | number): Observable<CuentoDetallePayload> {
    const db = this.firestore.db();
    ...
    return from(getDoc(docRef)).pipe(
      map(docSnap => {
        ...
        return {
          cuento: { id: docSnap.id, ...data } as CuentoRegistro,
  ...
  mio(fresh = false): Observable<CuentoRegistro | null> {
    ...
    return from(getDocs(q)).pipe(
      map(snapshot => {
        ...
        return { id: primerDoc.id, ...primerDoc.data() } as CuentoRegistro;
      })
    );
  }

  guardar(datos: CuentoPayload): Observable<CuentoRegistro> {
    const usuarioActual = this.sesion.usuario();
    const db = this.firestore.db();
    const cuentosRef = collection(db, this.coleccionCuentos);
    ...
    return from((async () => {
      ...
      return { id: docRef.id, ...nuevoCuento } as CuentoRegistro;
    })());
  }
  ...
  listarComentarios(cuentoId: string | number): Observable<CuentoComentario[]> {
    const db = this.firestore.db();
    ...
    return from(getDocs(q)).pipe(
      map(snapshot => {
        const docs = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as CuentoComentario));
  ...
  agregarComentario(cuentoId: string | number, contenido: string): Observable<CuentoComentario> {
    const usuarioActual = this.sesion.usuario();
    const db = this.firestore.db();
    const comentariosRef = collection(db, 'cuento_comentarios');
    ...
    return from(addDoc(comentariosRef, nuevoComentario)).pipe(
      map(docRef => ({ id: docRef.id, ...nuevoComentario }) as CuentoComentario)
    );
  }
  ...
  listarReacciones(cuentoId: string | number): Observable<CuentoReaccion[]> {
    const db = this.firestore.db();
    const reaccionesRef = collection(db, 'cuento_reacciones');
    const q = query(reaccionesRef, where('cuento_id', '==', String(cuentoId)));
    
    return from(getDocs(q)).pipe(
      map(snapshot => snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as CuentoReaccion)))
    );
  }

  agregarReaccion(cuentoId: string | number, tipo: string): Observable<CuentoReaccion> {
    const usuarioActual = this.sesion.usuario();
    const db = this.firestore.db();
    ...
    return from(setDoc(reaccionDoc, nuevaReaccion)).pipe(
      map(() => ({ id: docId, ...nuevaReaccion }) as CuentoReaccion)
    );
  }
}
