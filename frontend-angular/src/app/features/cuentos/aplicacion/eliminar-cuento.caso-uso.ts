...
@Injectable()
export class EliminarCuentoCasoUso {
  ...
  async ejecutar(cuentoId: string): Promise<ResultadoComandoCuento> {
    const clave = this.claves.get(cuentoId) ?? crearClaveIdempotencia(`eliminar:${cuentoId}`);
    this.claves.set(cuentoId, clave);
    const resultado = await this.gateway.eliminar(cuentoId, clave);
