...
export const routes: Routes = [
  ...
  { path: 'recuperar-clave', component: RecuperarClave },
  {
    path: 'alumno', component: LayoutAlumno, canActivate: [authGuard, alumnoGuard], children: [
      ...
      { path: 'perfil', component: PerfilAlumno, data: { titulo: 'Mi perfil', descripcion: 'Datos personales, insignias y mochila digital.', endpoint: '/alumno/perfil', accion: { etiqueta: 'Actualizar perfil', endpoint: '/alumno/perfil', campos: [{ nombre: 'nombre_completo', etiqueta: 'Nombre completo' }, { nombre: 'email', etiqueta: 'Correo' }, { nombre: 'biografia', etiqueta: 'Biografia', tipo: 'textarea' }] } } },
      { path: 'perfil/editar', component: EditarPerfil },
      { path: 'desafios', component: MisDesafios, data: { titulo: 'Mis desafios', descripcion: 'Misiones disponibles para tu nivel y estado de tus entregas.', endpoint: '/misiones', accion: { etiqueta: 'Entregar evidencia', endpoint: '/misiones/{id}/entregar', campos: [{ nombre: 'id', etiqueta: 'ID de mision', tipo: 'number' }, { nombre: 'texto', etiqueta: 'Evidencia', tipo: 'textarea' }] } } },
      ...
      { path: 'canjes', component: MisCanjes, data: { titulo: 'Mis canjes', descripcion: 'Historial y codigos de premios digitales.', endpoint: '/tienda/canjes' } },
      { path: 'evaluaciones', component: ExamenLive, data: { titulo: 'Evaluaciones', descripcion: 'Examenes activos para tu nivel.', endpoint: '/evaluaciones/activas', accion: { etiqueta: 'Enviar respuestas', endpoint: '/evaluaciones/{id}/responder', campos: [{ nombre: 'id', etiqueta: 'ID del examen', tipo: 'number' }, { nombre: 'respuestas', etiqueta: 'Respuestas JSON: {"1":"A"}', tipo: 'json', valor: '{}' }] } } },
      { path: 'resultados', component: ResultadosExamen, data: { titulo: 'Mis resultados', descripcion: 'Puntajes obtenidos y examenes enviados.', endpoint: '/evaluaciones/resultados' } },
      { path: 'competencia', component: Votar, data: { titulo: 'Competencia en vivo', descripcion: 'Estado de la ronda y voto actual.', endpoint: '/competencia/estado', accion: { etiqueta: 'Enviar voto', endpoint: '/competencia/votar', campos: [{ nombre: 'puntuacion', etiqueta: 'Puntuacion (1-10)', tipo: 'number' }, { nombre: 'comentario', etiqueta: 'Comentario' }] } } },
      { path: 'competencia/tv', component: Tv },
      { path: 'cuentos', component: GaleriaProyectos, data: { titulo: 'Galeria de cuentos', descripcion: 'Historias creadas por la comunidad.', endpoint: '/cuentos', accion: { etiqueta: 'Guardar mi cuento', endpoint: '/cuentos', campos: [{ nombre: 'titulo', etiqueta: 'Titulo' }, { nombre: 'data_1', etiqueta: 'Contenido de la primera escena', tipo: 'textarea' }] } } },
      { path: 'cuentos/crear', component: CrearCuento },
      { path: 'cuentos/:id', component: VerCuento },
      { path: 'ranking', component: Ranking, data: { titulo: 'Ranking', descripcion: 'Clasificacion por nivel y tokens.', endpoint: '/ranking' } },
      ...
      { path: 'certificado', component: Certificado, data: { titulo: 'Certificado y carnet', descripcion: 'Datos listos para impresion y acreditacion.', endpoint: '/certificados' } },
  ...
  },
  {
    path: 'docente', component: LayoutDocente, canActivate: [authGuard, docenteGuard], children: [
      ...
      { path: 'insignias', component: GestionarInsignias, data: { titulo: 'Insignias', descripcion: 'Catalogo de reconocimientos del aula.', endpoint: '/docente/insignias', accion: { etiqueta: 'Crear insignia', endpoint: '/docente/insignias', campos: [{ nombre: 'nombre', etiqueta: 'Nombre' }, { nombre: 'descripcion', etiqueta: 'Descripcion' }, { nombre: 'imagen', etiqueta: 'Ruta de imagen' }] } } },
      { path: 'tienda', component: GestionarTienda, data: { titulo: 'Gestionar tienda', descripcion: 'Premios, existencias y canjes.', endpoint: '/tienda/administrar', accion: { etiqueta: 'Crear premio', endpoint: '/tienda/premios', campos: [{ nombre: 'nombre', etiqueta: 'Nombre' }, { nombre: 'descripcion', etiqueta: 'Descripcion' }, { nombre: 'precio', etiqueta: 'Precio', tipo: 'number' }, { nombre: 'stock', etiqueta: 'Stock', tipo: 'number' }, { nombre: 'categoria', etiqueta: 'Categoria', tipo: 'select', opciones: ['GENERAL','KIDS','TEENS','PRO'], valor: 'GENERAL' }, { nombre: 'tipo_entrega', etiqueta: 'Entrega', tipo: 'select', opciones: ['fisico','digital'], valor: 'fisico' }] } } },
      { path: 'evaluaciones', component: GestionarEvaluacion, data: { titulo: 'Evaluaciones', descripcion: 'Examenes, estados y banco de preguntas.', endpoint: '/evaluaciones', accion: { etiqueta: 'Crear evaluacion', endpoint: '/evaluaciones', campos: [{ nombre: 'titulo', etiqueta: 'Titulo' }, { nombre: 'nivel', etiqueta: 'Nivel', tipo: 'select', opciones: ['KIDS','TEENS','PRO'], valor: 'TEENS' }, { nombre: 'estado', etiqueta: 'Estado', tipo: 'select', opciones: ['borrador','activo','finalizado'], valor: 'borrador' }] } } },
      { path: 'evaluaciones/resultados', component: VerResultados },
      { path: 'competencia', component: CompetenciaControl, data: { titulo: 'Control de competencia', descripcion: 'Selecciona candidato, inicia votacion, cierra y premia.', endpoint: '/competencia/estado', accion: { etiqueta: 'Controlar ronda', endpoint: '/competencia/control', campos: [{ nombre: 'accion', etiqueta: 'Accion', tipo: 'select', opciones: ['candidato','iniciar','cerrar','premiar','reiniciar'], valor: 'candidato' }, { nombre: 'id_alumno', etiqueta: 'ID del alumno', tipo: 'number' }, { nombre: 'duracion', etiqueta: 'Segundos', tipo: 'number', valor: 60 }, { nombre: 'puntos', etiqueta: 'Premio', tipo: 'number' }] } } },
      { path: 'competencia/tv', component: Tv },
      ...
      { path: 'tokens', component: HistorialTokens, data: { titulo: 'Historial de tokens', descripcion: 'Auditoria de todos los movimientos.', endpoint: '/docente/historial-tokens' } },
    ],
  },
  { path: '**', redirectTo: '' },
];
